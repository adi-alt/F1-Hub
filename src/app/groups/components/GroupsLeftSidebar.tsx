...
export function GroupsLeftSidebar({ groups, onDiscover }: { groups: GroupSummary[]; onDiscover: () => void }) {
  ...
  return (
    <div className="space-y-4">
      <div>
        <p className="px-1 text-xs font-semibold uppercase tracking-wide text-neutral-500">My Communities</p>
        <motion.div initial="hidden" animate="show" className="mt-2 space-y-0.5">
          {groups.map((g, i) => (
            <motion.div key={g.id} initial={{ opacity: 0, x: -6 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.2, delay: Math.min(i, 8) * 0.03 }}>
              <Link
                href={groupHref(g.id)}
                className="flex items-center gap-2 rounded-lg border border-transparent px-2 py-1.5 text-sm text-neutral-300 transition hover:border-[var(--f1-line)] hover:bg-white/[0.04] hover:text-white"
              >
          ...
          {groups.length === 0 && <p className="px-2 text-xs text-neutral-600">You haven&apos;t joined any communities yet.</p>}
        </motion.div>
      </div>
